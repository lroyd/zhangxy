					      ...
					      pjmedia_delay_buf **p_b)
{
    ...
    PJ_ASSERT_RETURN(pool && samples_per_frame && clock_rate && channel_count && p_b, PJ_EINVAL);
    ...
    {
        /* Create WSOLA */
        status = pjmedia_wsola_create(pool, clock_rate, samples_per_frame, 1, PJMEDIA_WSOLA_NO_FADING, &b->wsola);
        if (status != PJ_SUCCESS)
	    	return status;
    ...
    status = pj_lock_create_recursive_mutex(pool, b->obj_name, &b->lock);
    if (status != PJ_SUCCESS)
		return status;
...
static void shrink_buffer(pjmedia_delay_buf *b, unsigned erase_cnt)
{
    ...
    pjmedia_circ_buf_get_read_regions(b->circ_buf, &buf1, &buf1len, &buf2, &buf2len);
    status = pjmedia_wsola_discard(b->wsola, buf1, buf1len, buf2, buf2len, &erase_cnt);
    ...
    if ((status == PJ_SUCCESS) && (erase_cnt > 0))
    {
		...
		pjmedia_circ_buf_set_len(b->circ_buf, pjmedia_circ_buf_get_len(b->circ_buf) - erase_cnt);

		PJ_LOG(5,(b->obj_name,"%d samples reduced, buf_cnt=%d", erase_cnt, pjmedia_circ_buf_get_len(b->circ_buf)));
    }
}
...
static void update(pjmedia_delay_buf *b, enum OP op)
{
    ...
    /* See if we need to shrink the buffer to reduce delay 看看我们是否需要缩小缓冲区以减少延迟 */
    if (op == OP_PUT && pjmedia_circ_buf_get_len(b->circ_buf) > b->samples_per_frame + b->eff_cnt)
    {
		...
		PJ_LOG(4,(b->obj_name,"Buffer size adjusted from %d to %d (eff_cnt=%d)", old_buf_cnt, pjmedia_circ_buf_get_len(b->circ_buf), b->eff_cnt));
    }
}

PJ_DEF(pj_status_t) pjmedia_delay_buf_put(pjmedia_delay_buf *b, pj_int16_t frame[])
{
    pj_status_t status;
    ...
    /* Overflow checking */
    if (pjmedia_circ_buf_get_len(b->circ_buf) + b->samples_per_frame > b->max_cnt)
    {
		unsigned erase_cnt;
        ...
        if (b->wsola) 
        {
		    ...
		    erase_cnt = pjmedia_circ_buf_get_len(b->circ_buf) +  b->samples_per_frame - b->max_cnt;
		...
		if (pjmedia_circ_buf_get_len(b->circ_buf) + b->samples_per_frame > b->max_cnt) 
		{
		    ...
		    PJ_LOG(4,(b->obj_name,"%sDropping %d eldest samples, buf_cnt=%d", (b->wsola? "Shrinking failed or insufficient. ": ""), erase_cnt, pjmedia_circ_buf_get_len(b->circ_buf)));
		}
    }
...
PJ_DEF(pj_status_t) pjmedia_delay_buf_get( pjmedia_delay_buf *b, pj_int16_t frame[])
{
    pj_status_t status = PJ_SUCCESS;
    ...
    if (pjmedia_circ_buf_get_len(b->circ_buf) < b->samples_per_frame)
    {
		...
		PJ_LOG(4,(b->obj_name,"Underflow, buf_cnt=%d, will generate 1 frame", pjmedia_circ_buf_get_len(b->circ_buf)));
		...
		if (!b->wsola || status != PJ_SUCCESS) 
		{
			...
			if (b->wsola)
		        PJ_LOG(4,(b->obj_name,"Error generating frame, status=%d", status));

		    pjmedia_circ_buf_read(b->circ_buf, frame, buf_len);
		    pjmedia_zero_samples(&frame[buf_len], b->samples_per_frame - buf_len);
